...
export const App = {
    ...
    onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        if (snap.exists()) {
          ...
          if (user.email.includes('admin')) { // Fallback for first run
            App.state.user = { Name: "Admin", Role: "SuperAdmin", UserID: user.uid };
            App.renderNav();
  ...
  renderNav: () => {
    ...
    const links = [{ id: 'dashboard', icon: 'LayoutDashboard', label: 'Dashboard' }, { id: 'leads', icon: 'Users', label: 'Leads' }];
    if (u.Role !== 'Employee') links.push({ id: 'admin', icon: 'Settings', label: 'Admin' });
    ...
    if (sidebar) {
      sidebar.innerHTML = `
        ...
        <div class="p-4 border-t border-slate-800"><div class="mb-4 px-4"><div class="text-white font-medium truncate">${u.Name}</div><div class="text-slate-500 text-xs capitalize">${u.Role}</div></div><button onclick="window.logout()" class="w-full flex items-center gap-3 px-4 py-2 text-sm text-red-400 hover:bg-red-500/10 rounded-lg"><i data-lucide="LogOut" class="w-4 h-4"></i> Logout</button></div>`;
    }
